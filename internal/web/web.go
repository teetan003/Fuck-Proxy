...
func (s *Server) startTask(restartXray bool) {
	...
	s.cron.AddJob(envStr("XUI_CADENCE_XRAY_RUNNING", "@every 1s"), job.NewCheckXrayRunningJob())

	// Check if xray needs to be restarted every 30 seconds
	s.cron.AddFunc(envStr("XUI_CADENCE_XRAY_RESTART", "@every 30s"), func() {
	...
	go func() {
		...
		s.cron.AddJob(envStr("XUI_CADENCE_XRAY_TRAFFIC", "@every 5s"), job.NewXrayTrafficJob())
	}()

	// Reconcile mtproto (mtg) sidecars and scrape their traffic
	mtJob := job.NewMtprotoJob()
	s.cron.AddJob(envStr("XUI_CADENCE_MTPROTO", "@every 10s"), mtJob)
	go mtJob.Run()

	// check client ips from log file every 10 sec
	s.cron.AddJob(envStr("XUI_CADENCE_CLIENT_IP_SCAN", "@every 10s"), job.NewCheckClientIpJob())

	s.cron.AddJob(envStr("XUI_CADENCE_NODE_HEARTBEAT", "@every 5s"), job.NewNodeHeartbeatJob())

	s.cron.AddJob(envStr("XUI_CADENCE_NODE_TRAFFIC", "@every 5s"), job.NewNodeTrafficSyncJob())

	// Outbound subscription auto-refresh (respects per-sub updateInterval)
	s.cron.AddJob(envStr("XUI_CADENCE_OUTBOUND_SUB", "@every 5m"), job.NewOutboundSubscriptionJob())
	...
	if (err == nil) && (isTgbotenabled) {
		...
		s.cron.AddJob(envStr("XUI_CADENCE_CHECK_HASH", "@every 2m"), job.NewCheckHashStorageJob())
	}
	...
	if s.cpuAlarmWanted() {
		s.cron.AddJob(envStr("XUI_CADENCE_CPU_ALARM", "@every 1m"), job.NewCheckCpuJob())
	}
}
